...
use std::mem;
use vm_memory::{Bytes, Address, GuestAddress, GuestMemory};
use kvm_ioctls::VcpuFd;
use kvm_bindings::{CpuId, kvm_fpu, kvm_regs, kvm_sregs, kvm_msr_entry, Msrs, kvm_lapic_state};
...
use linux_loader::{
    ...
    configurator::{
        BootConfigurator,
        BootParams,
        linux::LinuxBootConfigurator,
    },
};
use crate::{
    kvm::memory::GuestMemoryMmap,
    utils::byte_order,
    builder::InitrdConfig,
    arch::Error as ArchError,
};
...
pub enum RegError {
    ...
    SetBaseRegister(kvm_ioctls::Error),
...
pub fn setup_regs(vcpu: &VcpuFd, boot_ip: u64) -> Result<(), RegError> {
    ...
    vcpu.set_regs(&regs).map_err(RegError::SetBaseRegister)
}
...
fn write_gdt_table(table: &[u64], guest_mem: &GuestMemoryMmap) -> Result<(), RegError> {
    ...
    for (index, entry) in table.iter().enumerate() {
        let addr = guest_mem
            ...
            .ok_or(RegError::WriteGdt)?;
        guest_mem
            .write_obj(*entry, addr)
            .map_err(|_| RegError::WriteGdt)?;
    }
    Ok(())
}
...
fn configure_segments_flat(sregs: &mut kvm_sregs) {
    sregs.cs.base = 0;
    ...
    sregs.ds.g = 1;
    sregs.fs.base = 0;
    sregs.fs.limit = 0xffff_ffff;
    sregs.fs.g = 1;
    sregs.gs.base = 0;
    sregs.gs.limit = 0xffff_ffff;
    sregs.gs.g = 1;
    sregs.es.base = 0;
    sregs.es.limit = 0xffff_ffff;
    sregs.es.g = 1;
    sregs.ss.base = 0;
    sregs.ss.limit = 0xffff_ffff;
    sregs.ss.g = 1;
...
pub fn setup_sregs(vcpu: &VcpuFd, mem: &GuestMemoryMmap) -> Result<(), RegError> {
    let mut sregs: kvm_sregs = vcpu.get_sregs().map_err(RegError::GetStatusRegisters)?;

    configure_segments_and_sregs(&mut sregs, mem)?;
    setup_page_tables(&mut sregs, mem)?;

    // configure_segments_flat(&mut sregs);

    vcpu.set_sregs(&sregs).map_err(RegError::SetStatusRegisters)
}
