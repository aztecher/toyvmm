...
use vm_memory::{Bytes, Address, GuestMemory, GuestAddress};
...
impl<'a> DescriptorChain<'a> {
    ...
    fn is_valid(&self) -> bool {
        if self.mem.checked_offset(self.addr, self.len as usize).is_none() {
            false
...
impl Queue {
    ...
    pub fn is_valid(&self, mem: &GuestMemoryMmap) -> bool {
        ...
        if !self.ready {
            ...
            false
        } else if desc_table.checked_add(desc_table_size as u64).map_or(true, |v| !mem.address_in_range(v)) {
            println!(
            ...
            false
        } else if avail_ring.checked_add(avail_ring_size as u64).map_or(true, |v| !mem.address_in_range(v)) {
            println!(
            ...
            false
        } else if used_ring.checked_add(used_ring_size as u64).map_or(true, |v| !mem.address_in_range(v)) {
            println!(
    ...
    pub fn add_used(&mut self, mem: &GuestMemoryMmap, desc_index: u16, len: u32) {
        ...
        mem.write_obj(self.next_used.0, used_ring.unchecked_add(2)).unwrap();
    }
}
