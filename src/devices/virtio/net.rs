...
use crate::devices::{
    ...
    util::{
        ...
        net::tap::Tap,
        sys::net as netsys,
        sys::virtio::net as virtio_net_sys,
    },
};
use vm_memory::Bytes;
use crate::kvm::memory::GuestMemoryMmap;
use vmm_sys_util::{
    errno::Error as SysError,
    eventfd::EventFd,
};
...
pub enum NetError {
    ...
    CreateKillEventFd(std::io::Error),
    /// Cloning kill eventfd failed.
    CloneKillEventFd(std::io::Error),
            ...
            loop {
                match next_desc {
                    Some(desc) => {
                        ...
                        let read_result = self.mem.read(&mut frame[read_count..limit as usize], desc.addr);
                        match read_result {
                            Ok(sz) => {
                                read_count += sz;
                            }
...
impl Net {
    ...
    pub fn new(
        ...
        let kill_evt = EventFd::new(libc::EFD_NONBLOCK).map_err(NetError::CreateKillEventFd)?;
        ...
        tap.set_offload(
            netsys::TUN_F_CSUM | netsys::TUN_F_UFO | netsys::TUN_F_TSO4 | netsys::TUN_F_TSO6,
        ).map_err(NetError::TapSetOffload)?;

        let vnet_hdr_size
            = mem::size_of::<virtio_net_sys::virtio_net_hdr_v1>() as i32;
        tap.set_vnet_hdr_size(vnet_hdr_size).map_err(NetError::TapSetVnetHdrSize)?;
        tap.enable().map_err(NetError::TapEnable)?;

        let avail_features = 1 << virtio_net_sys::VIRTIO_NET_F_GUEST_CSUM
            | 1 << virtio_net_sys::VIRTIO_NET_F_CSUM
            | 1 << virtio_net_sys::VIRTIO_NET_F_GUEST_TSO4
            | 1 << virtio_net_sys::VIRTIO_NET_F_GUEST_UFO
            | 1 << virtio_net_sys::VIRTIO_NET_F_HOST_TSO4
            | 1 << virtio_net_sys::VIRTIO_NET_F_HOST_UFO
            | 1 << virtio_net_sys::VIRTIO_F_VERSION_1;
        Ok(Net {
...
impl VirtioDevice for Net {
    ...
    fn activate(
        ...
        if queues.len() != 2 || queue_evts.len() != 2 {
            println!("net: expected 2 queues, got {}", queues.len());
