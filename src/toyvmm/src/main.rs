...
mod tests {
    ...
    pub fn generate_config_with_features(kernel_image_path: &str, rootfs_path: &str) -> String {
        format!(
            r#"{{
                "boot-source": {{
                    "kernel_path": "{}",
    ...
    fn generate_config_file_with_features(kernel_image_path: &str, rootfs_path: &str) -> TempFile {
        let config = generate_config_with_features(kernel_image_path, rootfs_path);
        let config_file = TempFile::new().unwrap();
        config_file.as_file().write_all(config.as_bytes()).unwrap();
        config_file
    }
    ...
    #[test]
    fn test_vm_run_command_with_features() {
        let kernel_image_path = TempFile::new().unwrap();
        let rootfs_file = TempFile::new().unwrap();
        let config_file = generate_config_file_with_features(
            kernel_image_path.as_path().to_str().unwrap(),
            rootfs_file.as_path().to_str().unwrap(),
        );
        let args = vec![
